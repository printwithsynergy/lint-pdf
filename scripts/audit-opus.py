...
You will be given:
  ...
  2. (Optional) A STRUCTURAL EVIDENCE block — parsed PDF internals \
     (font embedding flags, ICC profiles, encryption status, XMP \
     metadata presence, output intents, spot color spaces, AcroForm / \
     OCG presence). When this block is present, treat it as ground \
     truth for findings that vision can't verify (font embedding, ICC, \
     encryption, etc.) — DO NOT mark those uncertain when the evidence \
     is shown.
  3. A list of findings produced by an automated rule-based engine.
...
Be calibrated and conservative:
  - Only mark "disagree" if you can see in the PDF (or in the \
structural-evidence block) that the engine's claim is wrong.
  - Mark "uncertain" only when neither the PDF rendering NOR the \
structural-evidence block can settle the question.
  - Default to "agree" when the issue is plausibly visible in the PDF, \
confirmed by the structural-evidence block, or is a metadata/structural \
claim that's reasonable to trust the engine on.
...
def call_opus(
    ...
    `structural_evidence` (PR B Slot 2A) is the parsed-PDF metadata dict
    the orchestrator stashes on ``Job.structural_evidence``. When present
    it's spliced in as a third content block so Opus can adjudicate
    structural findings (font embedding, ICC, encryption, XMP, output
    intents, spot CS) that vision can't verify on the rendered PDF alone.
    ...
    structure_block: list[dict[str, Any]] = []
    if structural_evidence:
        structure_block = [
            {
                "type": "text",
                "text": (
                    "STRUCTURAL EVIDENCE (parsed from the PDF internals; not "
                    "visible in rendering — use this to adjudicate findings "
                    "about font embedding, ICC profiles, encryption, XMP "
                    "metadata, output intents, and spot color spaces):\n"
                    + json.dumps(structural_evidence, indent=2, default=str)
                ),
            }
        ]
...
def audit_one(name: str, pdf: Path, poll: Path, out_dir: Path) -> FixtureAudit:
    ...
    # already need ~10 K output budget. PR B Slot 2A added stricter
    # tiers for very-large-finding fixtures (250+) where Amalgam-class
    ...
    # PR B Slot 2A: extract structural evidence from poll.json (added
    # by the orchestrator's _build_structural_evidence step). Older
    # smoke runs that pre-date the engine change don't have the field;
    # the audit gracefully runs without it.
    structural_evidence: dict[str, Any] | None = None
    try:
        poll_data = json.loads(poll.read_text())
        se = poll_data.get("structural_evidence")
        if isinstance(se, dict) and se:
            structural_evidence = se
    except (OSError, json.JSONDecodeError):
        structural_evidence = None

    raw, parsed = call_opus(
        pdf_blocks,
        findings_json,
        max_tokens=max_tokens,
