...
1. **Entry points** (forward-looking): packages declare plugins via
   ``[project.entry-points."lintpdf.plugins"]`` in their pyproject.toml.
...
2. **Decorator fallback** (existing): the legacy
   ``@register_ai_analyzer`` decorator in ``lintpdf.ai.registry`` adds
...
if TYPE_CHECKING:
    from lintpdf.plugin.protocol import Analyzer

logger = logging.getLogger(__name__)

ENTRY_POINT_GROUP = "lintpdf.plugins"
...
def discover_legacy_ai() -> list[Analyzer]:
    ...
    try:
        from lintpdf.ai.registry import get_all_ai_analyzers
        from lintpdf.plugin.host import LegacyAIAdapter
