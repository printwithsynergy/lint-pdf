...
1. **`default_services_for_saas()`** — constructs a concrete ``Services``
   ...
   (``siftpdf.audit.metering``, ``siftpdf.ai.cost_cap``,
   ``siftpdf.ai.gpu_client``, ``siftpdf.conformance.verapdf_client``,
...
def _wrap_metering() -> Any:
    try:
        from siftpdf.audit import metering as _m
    except ImportError:
        logger.info("siftpdf.audit.metering unavailable; using no-op")
...
def _wrap_cost_cap() -> Any:
    try:
        from siftpdf.ai import cost_cap as _cc
    except ImportError:
        logger.info("siftpdf.ai.cost_cap unavailable; using no-op")
...
def _wrap_gpu_client() -> Any:
    try:
        ...
        logger.info("siftpdf.ai.gpu_client unavailable")
        return None
...
def _wrap_renderer() -> Any:
    try:
        from siftpdf.rendering import render_all_pages, render_page_to_image
    except ImportError:
        logger.info("siftpdf.rendering unavailable")
        return None
...
def _wrap_verapdf() -> Any:
    try:
        from siftpdf.conformance import verapdf_client as _v
    except ImportError:
        logger.info("siftpdf.conformance.verapdf_client unavailable")
        return noop_verapdf()
    ...
    class _VeraPDFWrap:
        def is_configured(self) -> bool:
            try:
                ...
                from siftpdf.config import get_settings
...
def _wrap_database() -> Any:
    try:
        from siftpdf.api import database as _db
    except ImportError:
        logger.info("siftpdf.api.database unavailable")
        return None
...
def _wrap_storage() -> Any:
    try:
        from siftpdf.api.storage import get_storage_backend
    except ImportError:
        logger.info("siftpdf.api.storage unavailable; using no-op")
        return noop_storage()
...
def _wrap_tenants() -> Any:
    try:
        from siftpdf.tenants import config_resolver, entitlements
    except ImportError:
        logger.info("siftpdf.tenants.* unavailable; using no-op")
        return noop_tenants()
    ...
    class _TenantsWrap:
        ...
        def get_historical_quality_data(
            ...
            no longer imports siftpdf.api.models directly — closing the
            last eta6 violation.
            """
            try:
...
class LegacyAIAdapter:
    ...
    def manifest(self) -> PluginManifest:
        ...
        return PluginManifest(
            id=f"siftpdf.legacy.{cls.__module__}.{cls.__name__}",
