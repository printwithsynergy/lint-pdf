...
from siftpdf.api.models import BrandProfile, Job, JobStatus, Tenant
...
def _channel_cache_key(
    ...
    see ``siftpdf.reports.separation_renderer.channel_cache_key``.
    """
    from siftpdf.reports.separation_renderer import channel_cache_key
...
def _tac_runs_cache_key(
    ...
    from siftpdf.reports.separation_renderer import tac_runs_cache_key
...
async def get_page_tile(
    ...
    # rejects the file.
    from siftpdf.rendering import OCGError, render_page_to_image
...
async def get_separation_channel(
    ...
    from siftpdf.reports.separation_renderer import render_separation_channel
...
def _render_tac_with_runs(
    *,
    ...
    from siftpdf.reports.separation_renderer import render_tac_heatmap
...
def _build_viewer_config(
    *,
    ...
    (``siftpdf.overrides.OverridesEnvelope`` serialised to a dict). When
    ...
    shapes.
    """
    from siftpdf.reports.service import BrandingContext, resolve_branding
    ...
    # the job or minted the token. See ``siftpdf.overrides.envelope``
    ...
    from siftpdf.api.config import get_settings as _cdn_settings
...
async def get_tile_warming_status(
    ...
    ``siftpdf.viewer.warm_tiles`` Celery task. The viewer polls this
...
#: ``siftpdf.queue.tasks._CAPABILITY_ANALYZERS``.
...
async def fill_job_capability(
    ...
    from siftpdf.queue.tasks import fill_capability
...
async def _sample_densitometer(
    ...
    *,
    ...
    """
    from siftpdf.reports.separation_renderer import sample_densitometer
...
async def _get_layer_tile_bytes(
    ...
    from siftpdf.rendering import render_isolated_layer_tile
...
async def set_verdict(
    ...
    if previous_verdict != body.verdict:
        from siftpdf.webhooks.events import fire_job_state_changed, fire_verdict_changed
...
async def public_share(
    ...
    from siftpdf.email.service import send_report
