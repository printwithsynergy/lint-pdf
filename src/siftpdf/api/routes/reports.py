...
from siftpdf.api.models import BrandProfile, BrandProfileType, Job, JobFinding, ReportToken, Tenant
...
# generator dispatch in ``siftpdf.reports.service``.
...
class GenerateReportsRequest(BaseModel):
    ...
    # See ``siftpdf.overrides.OverridesEnvelope``.
...
async def generate_reports(  # skipcq: PY-R1000
    ...
    from siftpdf.overrides.envelope import ReportOverrides
    ...
    from siftpdf.reports.service import ReportDetailLevel
    ...
    if minted:
        from siftpdf.webhooks.events import fire_job_state_changed, fire_report_minted
...
async def _serve_report_by_extension(
    ...
    if download:
        ...
        if record.brand_mode == "anonymous":
            from siftpdf.reports.service import build_anonymous_filename
