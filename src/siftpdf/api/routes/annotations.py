...
async def create_annotation_auth(
    ...
    from siftpdf.webhooks.events import fire_annotation_created, fire_job_state_changed
...
async def delete_annotation_auth(
    ...
    from siftpdf.webhooks.events import fire_annotation_deleted, fire_job_state_changed
...
def _capture_visitor(token: str, email: str, request: Request, db: Session) -> None:
    ...
    if first_visit:
        try:
            rec = db.query(ReportToken).filter(ReportToken.token == token).first()
            if rec is not None:
                from siftpdf.webhooks.events import fire_share_link_visited
...
def _fan_out_comment_email(
    *,
    ...
    """
    try:
        from siftpdf.email.service import send_annotation_comment
...
async def create_comment_auth(
    ...
    from siftpdf.webhooks.events import fire_comment_created, fire_job_state_changed
