...
from siftpdf.api.middleware import check_burst_rate_limit, check_rate_limit
...
from siftpdf.api.upload_security import PDF_TYPES, validate_upload_streaming
from siftpdf.tenants.models import RATE_LIMIT_WARN_THRESHOLD
...
_overrides_param = Form(
    ...
    description=(
        ...
        "``share`` — see ``siftpdf.overrides.OverridesEnvelope``. All "
...
def _send_rate_warning_if_needed(tenant: Tenant, usage: object) -> None:  # skipcq: PY-R1000
    ...
    """
    from siftpdf.api.middleware import UsageInfo, get_redis_client
    from siftpdf.tenants.models import RATE_LIMIT_OVERAGE_THRESHOLD
    ...
    try:
        if threshold == RATE_LIMIT_OVERAGE_THRESHOLD and usage.overage_enabled:
            from siftpdf.email.service import send_overage_started
            ...
            from siftpdf.email.service import send_rate_limit_warning
...
async def submit_job(  # skipcq: PY-R1000
    ...
    if overrides:
        ...
        from siftpdf.overrides import OverridesEnvelope as _OverridesEnvelope
    ...
    from siftpdf.billing.file_quota import check_and_consume_file_quota
    ...
    from siftpdf.profiles.resolver import (
    ...
    from siftpdf.api.gates import plan_upgrade_required
    ...
    if source_enum is PreflightSource.EXTERNAL and external_report is not None:
        ...
        from siftpdf.imports.detect import (
            detect_format,
            parser_for_format,
        )
    ...
    # ------------------------------------------------------------------
    from siftpdf.reports.service import BrandMode, parse_brand_param
            ...
            try:
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        "'anonymous', 'siftpdf', or a BrandProfile UUID."
                    ),
                ) from exc
            profile_row = (
    ...
    if brand_spec_id is not None:
        ...
        from siftpdf.brand_specs import storage as _brand_storage
    ...
    if workflow_id is not None:
        from siftpdf.tenants.toggle_models import Workflow as _Workflow
    ...
    if jdf_file is not None:
        try:
            ...
            from siftpdf.integrations.jdf_parser import params_to_overrides, parse_jdf
...
def _hydrate_job_response(db: Session, job: Job) -> JobResponse:
    ...
    if job.status == JobStatus.COMPLETE and job.result_json:
        ...
        from siftpdf.api.schemas import AuditVerdict
        from siftpdf.decisions.service import latest_active_for_finding
        ...
        from siftpdf.decisions.service import list_for_job
        ...
        from siftpdf.api.config import get_settings as _get_settings
        from siftpdf.api.models import BrandProfile, ReportToken, Tenant
