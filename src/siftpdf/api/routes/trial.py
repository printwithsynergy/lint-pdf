...
from siftpdf.api.auth import verify_admin_key
...
async def submit_trial(
    ...
    from siftpdf.api.storage import get_storage
    from siftpdf.api.upload_security import ANY_SAFE_TYPES, validate_upload_streaming
    ...
    try:
        from siftpdf.email.service import get_email_client

        if get_email_client() is not None:
            from siftpdf.email.service import _send
...
async def send_trial_report(
    ...
    if report_urls:
        from siftpdf.email.service import send_trial_report_email
