...
from siftpdf.api.auth import generate_api_key, hash_api_key, verify_admin_key
...
from siftpdf.api.routes.branding import EDGE_HOSTNAME
...
class UpdateEntitlementsRequest(BaseModel):
    ...
    def _ai_features_are_known(cls, v: list[str] | None) -> list[str] | None:
        if v is None:
            return None
        from siftpdf.tenants.entitlements import AI_FEATURE_FLAGS
...
async def update_tenant_plan(
    ...
    from siftpdf.tenants.models import PLAN_LIMITS, TenantPlan
    ...
    if previous_plan != str(new_plan):
        from siftpdf.webhooks.events import fire_tenant_plan_changed
...
async def create_tenant(
    ...
    from siftpdf.reports.service import _LINTPDF_DEFAULT_LOGO
...
class AdminUsageResponse(BaseModel):
    """Per-tenant usage snapshot returned to admin clients.

    Mirrors the fields returned by the tenant-facing ``/api/v1/usage``
    endpoint (see ``routes/usage.py``) so the dashboard's
    ``/api/lintpdf/usage`` proxy can hand this payload straight through
...
async def get_tenant_usage(
    ...
    """Return the same usage snapshot as ``/api/v1/usage`` but keyed
    on an explicit tenant id (admin-only).

    The tenant-facing ``/api/v1/usage`` endpoint resolves its tenant
    from the API key's ``Authorization: Bearer`` header. The app's
    ``/dashboard/usage`` page is rendered against the user's dashboard
    session, not a tenant API key, so the plugin route proxying it
    can't supply a per-tenant Bearer token. This endpoint lets the
    proxy pass the admin key + the resolved engine tenant id and get
    the correct per-tenant numbers back.
    """
    from siftpdf.api.middleware import build_usage_info, get_current_usage
...
async def admin_list_custom_domains(
    ...
    from siftpdf.api.models import BrandProfile
    from siftpdf.api.schemas import AdminCustomDomainRow
...
async def admin_prewarm_custom_domains(
    ...
    """
    from siftpdf.api.models import BrandProfile
    from siftpdf.queue.tasks import prewarm_edge_cert
...
async def get_job_detail(
    ...
    from siftpdf.api.models import ReportToken
...
async def get_tenant_profile(
    ...
    from siftpdf.profiles import storage as _profile_storage
    from siftpdf.profiles.registry import ProfileRegistry
    from siftpdf.profiles.schema import PreflightProfile
...
async def admin_rerun_audit(
    ...
    from siftpdf.queue.tasks import run_customer_audit
...
async def get_tenant_ai_status(
    ...
    from siftpdf.ai.config import get_or_create_ai_config
...
async def update_tenant_ai(
    ...
    from siftpdf.ai.config import admin_update_ai_config
    from siftpdf.api.models import AIBillingMode
...
async def list_audit_jobs(
    ...
    from siftpdf.api.models import JobFinding, JobImportedReport, ReportToken
...
async def refresh_imported_report_presigned_url(
    ...
    from siftpdf.api.models import JobImportedReport
...
async def admin_list_webhook_deliveries(
    ...
    """
    from siftpdf.api.models import WebhookDelivery
...
async def admin_replay_webhook_delivery(
    ...
    """
    from siftpdf.api.models import WebhookDelivery, WebhookEndpoint
    ...
    from siftpdf.queue.tasks import dispatch_webhook
...
async def admin_list_webhooks_for_tenant(
    ...
    from siftpdf.api.routes.webhooks import (
...
async def admin_create_webhook_for_tenant(
    ...
    from siftpdf.api.routes.webhooks import create_webhook_for_tenant
...
async def admin_delete_webhook_for_tenant(
    ...
    from siftpdf.api.routes.webhooks import delete_webhook_for_tenant
...
async def admin_test_webhook_for_tenant(
    ...
    from siftpdf.api.routes.webhooks import test_webhook_for_tenant
...
async def list_admin_system_profiles(
    ...
    from siftpdf.api.models import SystemProfile
...
async def set_tenant_default_profile(
    ...
    """
    from siftpdf.profiles.resolver import profile_exists_for_tenant
