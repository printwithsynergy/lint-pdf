...
class Job(Base):
    ...
    # Universal per-call override envelope (see ``siftpdf.overrides``).
    ...
    # via :func:`siftpdf.brand_specs.resolver.resolve_brand_spec_for_job`
...
class JobFinding(Base):
    ...
    # Populated by ``siftpdf.audit.internal.InternalAuditor`` during
    # dev/QA runs (via the admin health toolbox) and by
    # ``siftpdf.audit.claude.ClaudeAuditor`` during production runs
    ...
    # ``siftpdf.ai.explain.explain_finding`` after a successful Claude
...
class PlanLimitOverride(Base):
    ...
    :func:`siftpdf.tenants.entitlements.resolve_entitlements` for the
...
class ReportToken(Base):
    ...
    # ``siftpdf.overrides``). The viewer config endpoint reads this dict
...
class AIUsageLog(Base):
    ...
    # :func:`siftpdf.audit.metering.record_usage`. Sub-cent calls
