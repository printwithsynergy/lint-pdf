...
from siftpdf.profiles.icc.gamut_boundary import (  # noqa: E402
...
def _sample_srgb_grid(steps: int = 33) -> list[tuple[float, float, float]]:
    ...
    from siftpdf.analyzers.gamut_analyzer import srgb_to_lab
...
def _sample_rgb_profile_grid(
    profile_bytes: bytes,
    steps: int = 33,
) -> list[tuple[float, float, float]]:
    """Sample an RGB ICC profile at a regular grid and convert to Lab."""
    from PIL import Image, ImageCms

    rgb_profile = ImageCms.getOpenProfile(io.BytesIO(profile_bytes))
    lab_profile = ImageCms.createProfile("Lab")
    transform = ImageCms.buildTransform(
        rgb_profile,
        lab_profile,
    ...
    for r in values:
        for g in values:
            for b in values:
                ...
                src = Image.new("RGB", (1, 1), (r_b, g_b, b_b))
                lab_img = ImageCms.applyTransform(src, transform)
                px = lab_img.getpixel((0, 0))
...
def build_from_rgb_icc(
    profile_path: Path,
    condition_slug: str,
    condition_name: str,
) -> None:
    """Build gamut mesh from an RGB ICC profile file."""
    profile_bytes = profile_path.read_bytes()
    logger.info(
        "Sampling RGB grid from %s (condition: %s)...",
        profile_path.name,
        condition_slug,
    )

    lab_points = _sample_rgb_profile_grid(profile_bytes, steps=33)
    logger.info("Collected %d Lab samples", len(lab_points))
...
def main() -> None:
    ...
    parser.add_argument("--rgb", action="store_true", help="Treat profile as RGB (not CMYK)")
    ...
    if args.srgb:
        ...
        if args.rgb:
            build_from_rgb_icc(args.profile, args.condition, name)
        else:
