...
class PreflightOrchestrator:
    ...
    def run(self, pdf_bytes: bytes) -> PreflightResult:
        ...
        try:
            ...
            from siftpdf.plugin.host import default_services_for_saas
        ...
        if self._plan.conformance == "pdfx4":
            ...
            from siftpdf.conformance.pdfx1a import PdfX1aValidator

            validator = PdfX1aValidator()
            raw_findings.extend(validator.validate(document, events, raw_findings))
        elif self._plan.conformance in ("pdfx3", "pdfx32003"):
            from siftpdf.conformance.pdfx3 import PdfX3Validator

            validator = PdfX3Validator()
            raw_findings.extend(validator.validate(document, events, raw_findings))
        elif self._plan.conformance in ("pdfa1b", "pdfa2b", "pdfa3b"):
            from siftpdf.conformance.pdfa import PdfAValidator
        ...
        from siftpdf.conformance.verapdf_runner import run_verapdf_checks
    ...
    def _run_ai_analyzers(  # skipcq: PY-R1000
        ...
        try:
            from siftpdf.ai.registry import get_ai_analyzers
    ...
    def _create_analyzers(self) -> list[Any]:
        ...
        from siftpdf.analyzers.seal_zone_keepout import SealZoneKeepoutAnalyzer
        ...
        # ``siftpdf.brand_specs.resolver``) is the authoritative
        ...
        if t.ecg_mode:
            try:
                from siftpdf.analyzers.ecg_analyzer import EcgAnalyzer
        ...
        try:
            from siftpdf.analyzers.standards_compliance import StandardsComplianceAnalyzer
        ...
        if t.gamut_check and t.target_output_condition:
            try:
                from siftpdf.analyzers.gamut_analyzer import GamutAnalyzer
    ...
    def _apply_overrides_and_filter(self, findings: list[Finding]) -> list[Finding]:
        ...
        from siftpdf.rules.engine import CheckContext, evaluate_conditions
