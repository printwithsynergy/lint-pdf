"""Backwards-compat shim — page rendering moved to ``siftpdf.rendering``.

Phase 3c moved this module from ``siftpdf.ai.rendering`` to
``siftpdf.rendering`` because it's a pure CPU helper (pdf2image /
...
non-AI analyzers can import them directly from ``siftpdf.rendering``.
...
from siftpdf.rendering import (  # noqa: F401
    OCGError,
