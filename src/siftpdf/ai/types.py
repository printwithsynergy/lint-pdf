...
def get_db_session() -> Any:
    """Re-export of ``siftpdf.api.database.get_db_session``.

    Lazy-imported so that an OSS host stripping ``siftpdf.api.*``
    can still import ``siftpdf.ai.types`` (GPU + AIConfig surface)
    ...
    from siftpdf.api.database import get_db_session as _get
...
if TYPE_CHECKING:
    ...
    # so the file never names ``siftpdf.ai.gpu_client``.
