...
def run_customer_audit(db: Any, job: Any, job_id: str, *, force: bool = False) -> int:
    ...
    ``siftpdf.audit.claude.ClaudeAuditor``). On API failure the caller
    ...
    """
    from siftpdf.api.models import JobFinding, Tenant
    ...
    from siftpdf.audit.quota import current_month_usage_cents, is_over_quota
    ...
    from siftpdf.audit.claude import ClaudeAuditor
...
def _auto_generate_reports(
    ...
    from siftpdf.api.models import BrandProfile, JobFinding, ReportToken, Tenant
...
def run_preflight(
    ...
    try:
        ...
        from siftpdf.api.database import get_db_session
        ...
        try:
            ...
            # ------------------------------------------------------------------
            from siftpdf.api.models import PreflightSource
            ...
            if jdf_overrides:
                ...
                from siftpdf.profiles.schema import ThresholdConfig
            ...
            if ai_enabled is not None or ai_categories or ai_features:
                from siftpdf.profiles.schema import AIFeatureConfig
            ...
            if profile.ai and profile.ai.enabled:
                try:
                    from siftpdf.ai.access import get_ai_config
            ...
            try:
                from siftpdf.api.middleware import get_redis_client
                from siftpdf.api.models import TenantColorConfig
                from siftpdf.profiles.icc.pantone_cache import get_overrides, set_overrides
            ...
            try:
                from siftpdf.brand_specs.resolver import resolve_brand_spec_for_job
            ...
            # state.
            try:
                from siftpdf.analyzers.dieline import (
                    detect_dieline,
                    result_to_json,
                )
            ...
            try:
                from siftpdf.analyzers.art_size import (
                    compute_art_size,
                )
                from siftpdf.analyzers.art_size import (
                        ...
                        try:
                            ...
                            with pikepdf.open(io.BytesIO(pdf_bytes)) as pdf:
                                ...
                                if tb is not None and len(tb) == 4:
                                    ...
                                    from siftpdf.analyzers.dieline import DielineResult
            ...
            try:
                from siftpdf.queue.audit_tasks import (
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.queue.tasks.reap_stale_jobs",
)
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.queue.tasks.cleanup_expired_reports",
)
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.edge.prewarm_cert",
    time_limit=60,
    soft_time_limit=50,
    ignore_result=True,
    max_retries=1,
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.queue.tasks.probe_pending_custom_domains",
)
def probe_pending_custom_domains() -> dict[str, Any]:
    ...
    """
    from siftpdf.api.database import get_db_session
    from siftpdf.api.models import BrandProfile, Tenant
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.viewer.prerender_tiles",
    time_limit=120,
    soft_time_limit=110,
    ignore_result=True,
)
...
def _dispatch_tenant_webhooks(
    ...
    Funnels through ``siftpdf.webhooks.events.emit_event`` so every
...
def dispatch_webhook(
    ...
    from siftpdf.api.database import SessionLocal
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.queue.tasks.sweep_webhook_deliveries",
)
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.queue.tasks.process_approval_timeouts",
)
def process_approval_timeouts() -> dict[str, Any]:
    ...
    """
    from siftpdf.api.database import get_db_session
    from siftpdf.approvals.service import process_timeouts
...
def _extract_essentials_dict(pdf_bytes: bytes) -> dict[str, Any]:
    ...
    analyzer pipeline.
    """
    from siftpdf.profiles.orchestrator import extract_viewer_essentials
...
def _finalize_non_engine_job(
    ...
    from siftpdf.api.models import JobStatus
...
def _run_external_preflight(
    *, self: Any, db: Any, job: Any, job_id: str, start: float
    ...
    """
    from siftpdf.api.models import JobImportedReport, default_capabilities
    from siftpdf.api.storage import get_storage
    from siftpdf.imports import parse_external_report
...
def _run_minimal_preflight(
    *, self: Any, db: Any, job: Any, job_id: str, start: float
    ...
    """
    from siftpdf.api.models import default_capabilities
    from siftpdf.api.storage import get_storage
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.viewer.fill_capability",
    max_retries=1,
...
def fill_capability(job_id: str, capability: str) -> dict[str, Any]:
    ...
    * Flips ``job.data_capabilities[capability] = True``.
    ...
    from siftpdf.api.database import get_db_session
    from siftpdf.api.models import Job
    from siftpdf.api.storage import get_storage
...
def _tile_s3_key(tenant_id: str, job_id: str, page_num: int, dpi: int) -> str:
    ...
    """
    from siftpdf.api.routes.viewer import _TILE_RENDER_VERSION
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="siftpdf.viewer.warm_tiles",
    max_retries=1,
    default_retry_delay=10,
