...
def create_celery_app(broker_url: str) -> Celery:
    ...
    app.conf.update(
        ...
        task_routes={
            "siftpdf.webhook.dispatch": {"queue": "webhooks"},
            "siftpdf.queue.tasks.cleanup_expired_reports": {"queue": "default"},
            "siftpdf.queue.tasks.probe_pending_custom_domains": {"queue": "default"},
            "siftpdf.queue.tasks.process_approval_timeouts": {"queue": "default"},
            "siftpdf.queue.tasks.reap_stale_jobs": {"queue": "default"},
            "siftpdf.queue.tasks.sweep_webhook_deliveries": {"queue": "default"},
            "siftpdf.queue.audit_tasks.drain_ai_audit_rerun_queue": {"queue": "default"},
        },
        beat_schedule={
            "cleanup-expired-reports": {
                "task": "siftpdf.queue.tasks.cleanup_expired_reports",
                "schedule": 86400.0,  # Daily (24 hours)
            },
            "probe-pending-custom-domains": {
                "task": "siftpdf.queue.tasks.probe_pending_custom_domains",
                "schedule": 300.0,  # Every 5 minutes
            },
            "process-approval-timeouts": {
                "task": "siftpdf.queue.tasks.process_approval_timeouts",
                "schedule": 600.0,  # Every 10 minutes
            },
            "reap-stale-jobs": {
                "task": "siftpdf.queue.tasks.reap_stale_jobs",
                "schedule": 300.0,  # Every 5 minutes — safety net
            },
            "sweep-webhook-deliveries": {
                "task": "siftpdf.queue.tasks.sweep_webhook_deliveries",
                "schedule": 86400.0,  # Daily
            },
            "drain-ai-audit-rerun-queue": {
                "task": "siftpdf.queue.audit_tasks.drain_ai_audit_rerun_queue",
                "schedule": 600.0,  # Every 10 minutes
            },
        },
    )
    ...
    # import because audit_tasks does ``from siftpdf.queue.app import
    # celery_app`` before this function has returned.)
    app.autodiscover_tasks(["siftpdf.queue"])
...
def _configure_worker_process(**_: Any) -> None:
    ...
    """
    from siftpdf.api.database import reset_db_state
    from siftpdf.api.logging_config import configure_logging
...
def _drain_rerun_queue() -> None:
    ...
    try:
        from siftpdf.queue.audit_tasks import audit_findings_async
    except Exception:
