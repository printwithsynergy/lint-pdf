# ISO 32000-2:2020 Chapter 9: Text

## Font Technology in PDF
Sections covered: 9.1-9.10 including Simple Fonts, Composite Fonts, Font Descriptors, Embedded Font Programs, and Text Extraction (ToUnicode CMaps)
...
of the glyph is determined by the text matrix (T ) in the text object, several text state parameters, and
m
...
T Text font size
fs
T Text rendering mode
mode
T Text rise
rise
T Text knockout
k
Except for the previously described T and T , these parameters are discussed further in subsequent
f fs
subclauses. (As described in 9.4, "Text objects", three additional text-related parameters may occur
only within a text object: T , the text matrix; T , the text line matrix; and T , the text rendering
m lm rm
...
The text knockout parameter, T , shall be set through the TK entry in a graphics state parameter
k
...
charSpace Tc Set the character spacing, T, to charSpace, which shall be a number
c
...
scale Tz Set the horizontal scaling, T , to (scale ÷ 100). scale shall be a number
h
specifying the percentage of the normal width. Initial value: 100 (normal
width).
leading TL Set the text leading, T, to leading, which shall be a number expressed in
l
unscaled text space units. Text leading shall be used only by the T*, ', and "
operators. Initial value: 0.
font size Tf Set the text font, Tf, to font and the text font size, T , to size. font shall be
fs
...
render Tr Set the text rendering mode, T , to render, which shall be an integer.
mode
Initial value: 0.
rise Ts Set the text rise, T , to rise, which shall be a number expressed in
rise
...
specified in a coordinate system that shall be defined by the text matrix, T but shall not be scaled by
m
the font size parameter, T .
fs
9.3.2 Character spacing
The character-spacing parameter, T, shall be a number specified in unscaled text space units (although
c
...
--- PAGE 317 ---
ISO 32000-2:2020(E)
it shall be subject to scaling by the T parameter if the writing mode is horizontal). When the glyph for
h
each character in the string is rendered, T shall be added to the horizontal or vertical component of the
c
...
positive value of T has the effect of expanding the distance between glyphs (see "Figure 56 —
c
Character spacing in horizontal writing"), whereas for vertical writing, a negative value of T has this
c
effect.
...
character (20h). The word-spacing parameter, T , shall be added to the glyph’s horizontal or vertical
w
displacement (depending on the writing mode). For horizontal writing, a positive value for T has the
w
effect of increasing the spacing between words. For vertical writing, a positive value for T decreases
w
...
The horizontal scaling parameter, T , adjusts the width of glyphs by stretching or compressing them in
h
...
spacing parameters T and T , as well as any positioning adjustments performed by the TJ operator.
c w
"Figure 58 — Horizontal scaling" shows the effect of horizontal scaling.
Figure 58 — Horizontal scaling
9.3.5 Leading
The leading parameter, T, shall be specified in unscaled text space units. It specifies the vertical
l
...
The text rendering mode, T , determines whether showing text shall cause glyph outlines to be
mode
...
9.3.7 Text rise
Text rise, T , shall specify the distance, in unscaled text space units, to move the baseline up or down
rise
...
Figure 60 — Text rise
9.3.8 Text knockout
The text knockout parameter, T (PDF 1.4), shall be a boolean value that determines what text elements
k
...
• T , the text matrix.
m
• T , the text line matrix.
lm
• T , the text rendering matrix, which is actually just an intermediate result that combines the
rm
effects of text state parameters, the text matrix (T ), and the current transformation matrix.
m
...
Operands Operator Description
— BT Begin a text object, initializing the text matrix, T , and the text line
m
matrix, T , to the identity matrix. Text objects shall not be nested;
lm
...
m
and the text state parameters T , T , and T , which together shall determine the transformation from
h rise
fs
...
current line by (t, t ). t and t shall denote numbers
x y x y
...
𝑥 𝑦
tx ty TD Move to the start of the next line, offset from the start of the
current line by (t, t ). As a side effect, this operator shall set
x y
...
t t Td
x y
a b c d e f Tm Set the text matrix, T , and the text line matrix, T :
m lm
𝑎 𝑏 0
𝑇 = 𝑇 = [𝑐 𝑑 0]
𝑚 𝑙𝑚
...
T and T shall be the identity matrix, [1 0 0 1 0 0].
m lm
Although the operands specify a matrix, they shall be passed
to T as six separate numbers, not as an array.
m
...
0 –T TD
l
where T denotes the current leading parameter in the text
l
state. The negative of T is used here because T is the text
l l
leading expressed as a positive number. Going to the next
line entails decreasing the y coordinate.
At the beginning of a text object, T shall be the identity matrix; therefore, the origin of text space shall
m
be initially the same as that of user space. The text-positioning operators, described in "Table 106 —
Text-positioning operators" alter T and thereby control the placement of glyphs that are subsequently
m
painted. Also, the text-showing operators, described in "Table 107 — Text-showing operators", update
T (by altering its e and f translation components) to take into account the horizontal or vertical
m
...
--- PAGE 324 ---
ISO 32000-2:2020(E)
Additionally, within a text object, a PDF processor shall keep track of a text line matrix, T , which
lm
captures the value of T at the beginning of a line of text. The text-positioning and text-showing
m
operators shall read and set T on specific occasions mentioned in "Table 106 — Text-positioning
lm
...
T*
string Tj
a a string " Move to the next line and show a text string, using a as the word
w c w
spacing and a as the character spacing (setting the corresponding
c
parameters in the text state). a and a shall be numbers expressed in
w c
...
that is, it shall translate the text matrix, T . The number shall be
m
...
combination of the text matrix, T , and the text state parameters T , T , and T . This determines how
m h rise
fs
...
0 𝑇 1
𝑟𝑖𝑠𝑒
T is a temporary matrix; conceptually, it is recomputed before each glyph is painted during a
rm
...
spacing parameters that apply. First, a combined displacement shall be computed, denoted by t in
x
horizontal writing mode or t in vertical writing mode (the variable corresponding to the other writing
y
...
T denotes a number in a TJ array, if any, which specifies a position adjustment
j
T and T denote the current text font size and horizontal scaling parameters in the graphics state
fs h
T and T denote the current character-and word-spacing parameters in the graphics state, if
c w
applicable
...
Operands Operator Description
w w d0 Set width information for the glyph and declare that the glyph description
x y
specifies both its shape and its colour.
NOTE 1 This operator name ends in the digit 0.
w denotes the horizontal displacement in the glyph coordinate system; it shall
x
be consistent with the corresponding width in the font’s Widths array. w shall
y
...
w w ll ll ur ur d1 Set width and bounding box information for the glyph and declare that the glyph
x y x y x y
...
y
be 0 (see 9.2.4, "Glyph positioning and metrics").
ll and ll denote the coordinates of the lower-left corner, and ur and ur denote
x y x y
...
shall define the same width, w, as a number, for all CIDs in the range c to c . Specifying a given CID
first last
