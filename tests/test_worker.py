...
from siftpdf.queue.app import create_celery_app
...
class TestCeleryAppCreation:
    ...
    def test_beat_schedule_has_cleanup(celery_app) -> None:
        ...
        assert task_conf["task"] == "siftpdf.queue.tasks.cleanup_expired_reports"
...
class TestWorkerInit:
    ...
    def test_on_worker_init_calls_init_db() -> None:
        ...
        with (
            patch("siftpdf.api.database.init_db") as mock_init_db,
            patch.dict("os.environ", {"LINTPDF_DATABASE_URL": "postgresql://test:5432/testdb"}),
        ):
            from siftpdf.queue.worker import _on_worker_init
    ...
    def test_worker_module_exports_app() -> None:
        ...
        from siftpdf.queue.worker import app
    ...
    def test_reset_db_state_drops_engine() -> None:
        ...
        from siftpdf.api import database as db_module
        ...
        with patch("siftpdf.api.database.create_engine", return_value=fake_engine):
    ...
    def test_worker_process_init_hook_resets_db_state() -> None:
        ...
        from siftpdf.queue.app import _configure_worker_process

        with (
            patch("siftpdf.api.database.reset_db_state") as mock_reset,
            patch("siftpdf.api.logging_config.configure_logging") as mock_log,
        ):
...
class TestRunPreflightTask:
    ...
    def test_job_not_found_returns_failed(self) -> None:
        ...
        with patch("siftpdf.api.database.get_db_session", return_value=mock_db):
    ...
    def test_successful_preflight_run(self) -> None:
        ...
        with (
            patch("siftpdf.api.database.get_db_session", return_value=mock_db),
            patch("siftpdf.api.storage.get_storage", return_value=mock_storage),
            patch("siftpdf.profiles.registry.ProfileRegistry") as MockRegistry,  # noqa: N806
            patch("siftpdf.profiles.orchestrator.PreflightOrchestrator") as MockOrch,  # noqa: N806
            patch("siftpdf.queue.tasks._dispatch_tenant_webhooks"),
        ):
    ...
    def test_r2_failure_falls_back_to_redis(self) -> None:
        ...
        with (
            ...
            patch("siftpdf.profiles.registry.ProfileRegistry") as MockReg,  # noqa: N806
            patch("siftpdf.profiles.orchestrator.PreflightOrchestrator") as MockOrch,  # noqa: N806
            patch("siftpdf.queue.tasks._dispatch_tenant_webhooks"),
        ):
    ...
    def test_complete_failure_no_pdf(self) -> None:
        ...
        with (
            patch("siftpdf.api.database.get_db_session", return_value=mock_db),
            patch("siftpdf.api.storage.get_storage", return_value=mock_storage),
            patch("siftpdf.api.middleware.get_redis_client", return_value=None),
        ):
...
class TestCleanupExpiredReports:
    ...
    @staticmethod
    def test_task_is_registered() -> None:
        from siftpdf.queue.tasks import cleanup_expired_reports

        assert cleanup_expired_reports.name == "siftpdf.queue.tasks.cleanup_expired_reports"
    ...
    def test_cleanup_returns_count() -> None:
        ...
        with (
            patch("siftpdf.api.database.get_db_session", return_value=mock_db),
            patch("siftpdf.api.storage.get_storage", return_value=mock_storage),
            patch("siftpdf.reports.service.ReportService", return_value=mock_service),
        ):
            from siftpdf.queue.tasks import cleanup_expired_reports
...
class TestDispatchTenantWebhooks:
    ...
    def test_dispatches_to_active_endpoints() -> None:
        from siftpdf.queue.tasks import _dispatch_tenant_webhooks
        ...
        with patch("siftpdf.queue.tasks.dispatch_webhook") as mock_dispatch:
