...
``siftpdf.audit.outage.is_outage()`` into a JSON status the viewer
...
class TestAiHealth:
    ...
    def test_ok_when_outage_flag_clear(client: TestClient) -> None:
        with patch("siftpdf.api.routes.ai_health.is_outage", return_value=False):
    ...
    def test_degraded_when_outage_flag_set(client: TestClient) -> None:
        with patch("siftpdf.api.routes.ai_health.is_outage", return_value=True):
            resp = client.get("/api/v1/ai/health")
            assert resp.status_code == 200
