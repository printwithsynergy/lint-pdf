"""Tests for StructureAnalyzer — LPDF_STRUCT_006-007 (Part 4 deepening).
...
class TestXFAForms:
    """Test LPDF_STRUCT_006: XFA forms detection."""

    @staticmethod
    def test_xfa_detected() -> None:
        """AcroForm with /XFA triggers LPDF_STRUCT_006."""
        doc = _make_document({"/AcroForm": {"/XFA": ["some", "xfa", "data"]}})
        findings = StructureAnalyzer().analyze(doc, [])
        xfa = [f for f in findings if f.inspection_id == "LPDF_STRUCT_006"]
        assert len(xfa) == 1
    ...
    def test_acroform_without_xfa_no_finding() -> None:
        """AcroForm without /XFA does not trigger LPDF_STRUCT_006."""
    ...
    def test_no_acroform_no_finding() -> None:
        """No AcroForm does not trigger LPDF_STRUCT_006."""
        doc = _make_document({})
        findings = StructureAnalyzer().analyze(doc, [])
        xfa = [f for f in findings if f.inspection_id == "LPDF_STRUCT_006"]
        assert len(xfa) == 0
    ...
    def test_xfa_none_no_finding() -> None:
        """AcroForm with /XFA=None does not trigger LPDF_STRUCT_006."""
...
class TestTaggedPDF:
    """Test LPDF_STRUCT_007: tagged PDF (structure tree)."""

    @staticmethod
    def test_mark_info_detected() -> None:
        """/MarkInfo in catalog triggers LPDF_STRUCT_007."""
        doc = _make_document({"/MarkInfo": {"/Marked": True}})
        findings = StructureAnalyzer().analyze(doc, [])
        tag = [f for f in findings if f.inspection_id == "LPDF_STRUCT_007"]
        assert len(tag) == 1
    ...
    def test_struct_tree_root_detected() -> None:
        """/StructTreeRoot in catalog triggers LPDF_STRUCT_007."""
    ...
    def test_no_tagging_no_finding() -> None:
        """Empty catalog does not trigger LPDF_STRUCT_007."""
        doc = _make_document({})
        findings = StructureAnalyzer().analyze(doc, [])
        tag = [f for f in findings if f.inspection_id == "LPDF_STRUCT_007"]
        assert len(tag) == 0
