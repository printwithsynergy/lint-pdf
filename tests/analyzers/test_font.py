...
class TestFontEmbedding:
    """Test LPDF_FONT_001: font embedding checks."""
...
class TestFontSubsetting:
    """Test LPDF_FONT_002: subsetting checks."""
    ...
    def test_subsetted_font_no_finding() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        sub_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_002"]
...
class TestStandard14:
    """Test LPDF_FONT_003: Standard 14 font detection."""
    ...
    def test_standard_14_advisory() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        std14_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_003"]
...
class TestType3:
    """Test LPDF_FONT_004: Type 3 font detection."""
    ...
    def test_type3_delay() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        t3_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_004"]
        assert len(t3_findings) == 1
        assert t3_findings[0].severity == Severity.WARNING


class TestCIDFonts:
    """Test LPDF_FONT_005 and LPDF_FONT_006: CID font checks."""
    ...
    def test_cid_missing_tounicode() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        cid_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_005"]
    ...
    def test_cid_missing_system_info() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        sys_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_006"]
        assert len(sys_findings) == 1


class TestFontEncoding:
    """Test LPDF_FONT_007: missing encoding."""
    ...
    def test_no_encoding_advisory() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        enc_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_007"]
...
class TestTrueTypeEmbedding:
    """Test LPDF_FONT_008: TrueType not embedded."""
    ...
    def test_truetype_not_embedded_aground() -> None:
        ...
        analyzer = FontAnalyzer()
        findings = analyzer.analyze(_make_doc_with_font(font), [])
        tt_findings = [f for f in findings if f.inspection_id == "LPDF_FONT_008"]
