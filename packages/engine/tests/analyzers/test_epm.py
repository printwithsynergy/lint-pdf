...
def _make_doc():
    return SemanticDocument(
        version="2.0", page_count=1, is_encrypted=False,
...
class TestEpmAnalyzer:
    def test_no_findings_for_cmy_only_file(self):
        ...
        events = [PathPaintingEvent(
            operator="f", page_num=1, operator_index=0,
    ...
    def test_pure_black_text_detected(self):
        events = [TextRenderedEvent(
            operator="Tj", page_num=1, operator_index=0,
            font_name="F1", font_size=12.0,
