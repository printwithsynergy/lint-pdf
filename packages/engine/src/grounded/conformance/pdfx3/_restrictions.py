...
def validate_restrictions(
    ...
    if isinstance(names, dict):
        ...
        if ef is not None:
            findings.append(Finding(
                inspection_id=f"{_PREFIX}-033", severity=Severity.AGROUND,
    ...
    if isinstance(acro_form, dict):
        ...
        if isinstance(fields, list) and len(fields) > 0:
            findings.append(Finding(
                inspection_id=f"{_PREFIX}-034", severity=Severity.AGROUND,
    ...
    if document.is_encrypted:
        findings.append(Finding(
            inspection_id=f"{_PREFIX}-037", severity=Severity.AGROUND,
    ...
    for page in document.pages:
        if page.trim_box is None and page.art_box is None:
            findings.append(Finding(
                inspection_id=f"{_PREFIX}-041", severity=Severity.AGROUND,
    ...
    if isinstance(oc_properties, dict):
        findings.append(Finding(
            inspection_id=f"{_PREFIX}-043", severity=Severity.AGROUND,
        ...
        if isinstance(xobjects, dict):
            for xobj_name, xobj_dict in xobjects.items():
                ...
                if "/OPI" in xobj_dict or "OPI" in xobj_dict:
                    findings.append(Finding(
                        inspection_id=f"{_PREFIX}-035", severity=Severity.AGROUND,
                ...
                if subtype in ("/PS", "PS"):
                    findings.append(Finding(
                        inspection_id=f"{_PREFIX}-040", severity=Severity.AGROUND,
                ...
                if "/Alternates" in xobj_dict or "Alternates" in xobj_dict:
                    findings.append(Finding(
                        inspection_id=f"{_PREFIX}-042", severity=Severity.AGROUND,
        ...
        if isinstance(ext_gstate, dict):
            for gs_name, gs_dict in ext_gstate.items():
                ...
                if tr is not None or tr2 is not None:
                    findings.append(Finding(
                        inspection_id=f"{_PREFIX}-038", severity=Severity.AGROUND,
    ...
    for event in events:
        if isinstance(event, ImagePlacedEvent) and event.filters and "LZWDecode" in event.filters:
            findings.append(Finding(
                inspection_id=f"{_PREFIX}-036", severity=Severity.AGROUND,
    ...
    if isinstance(aa, dict):
        for action in aa.values():
            if isinstance(action, dict) and (action.get("/S") == "/JavaScript" or action.get("S") == "JavaScript"):
                findings.append(Finding(
                    inspection_id=f"{_PREFIX}-032", severity=Severity.AGROUND,
