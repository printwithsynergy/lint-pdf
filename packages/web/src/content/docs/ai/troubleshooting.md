...
No. The core engine runs 500+ deterministic, rule-based checks and is not affected by AI features. AI inspections run alongside core engine checks and produce additional findings in the same report — the `source` field distinguishes them (`"engine"` vs. `"ai"`).
