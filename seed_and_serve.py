...
def seed():
    ...
    t0 = time.time()
    result = orchestrator_result = PreflightOrchestrator(
        profile, profile_id="full-preflight-ai"
    ).run(pdf_bytes)
    elapsed = time.time() - t0
    sm = result.summary
    print(
        f"  Done in {elapsed:.1f}s — {'PASS' if sm.passed else 'FAIL'} — "
    ...
    result_json = {
        ...
        "summary": {
            ...
            "warning_count": sm.warning_count,
            "advisory_count": sm.advisory_count,
            "passed": sm.passed,
    ...
    for fmt in ["html", "pdf"]:
        report_bytes = report_engine.generate(
            result, fmt, pdf_bytes=pdf_bytes, detail_level="comprehensive"
        )
